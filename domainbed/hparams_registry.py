...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('im21k', True, lambda r: True)
    _hparam('best_step_checkpoint', 1e-5, lambda r: 1e-5)
    _hparam('backbones', 'vit_base_patch16_224.orig_in21k',
            lambda r: r.choice(['vit_base_patch16_224.orig_in21k', 'vit_base_patch16_224.augreg_in1k', 'vit_base_patch16_224.mae', 'vit_huge_patch14_224.orig_in21k']))
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('penalty_anneal_iters', 1500,
                lambda r: int(r.uniform(0., 5000.)))
        ...
        _hparam('lmbda', 1e-1,
                lambda r: r.choice([1e-4, 1e-3, 1e-2, 1e-1, 1, 1e1, 1e2]))
    ...
    elif algorithm == "DoPrompt" or algorithm in ["PADA"]:
        # ! these are our supplemental hparams
    ...
    elif algorithm in ["HCVP"]:
        _hparam('hcvp_freeze_backbone', False, lambda r: False)
        _hparam('hcvp_DEEP', True, lambda r: True)
        _hparam('hcvp_modulation_select', 'simple',
                lambda r: r.choice(['simple', 'shared', 'separate']))
        _hparam('hcvp_dropout', 0.1, lambda r: 0.1)
        _hparam('hcvp_loss_coarse', 0.0, lambda r: 0.0)
        _hparam('hcvp_loss_fine', 0.0, lambda r: 0.0)
        _hparam('hcvp_loss_invariance', 0.0, lambda r: 0.0)
        _hparam('hcvp_temperature', 1.0, lambda r: 1.0)
    ...
    _hparam('lr_classifier', 5e-5, lambda r: 10**r.uniform(-5, -3.5))
    ...
    elif dataset in ['DomainNet', 'WILDSCamelyon', 'WILDSFMoW']:
