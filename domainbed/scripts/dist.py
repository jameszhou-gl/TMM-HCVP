...
def cal_feat_mean(network, loader, weights, device, domain=None, algorithm=None):
    ...
    feat_cls_mean = torch.zeros(
        network.num_classes, network.featurizer.n_outputs).cuda()
    ...
    for x, y in loader:
        ...
        if algorithm == "DomainPrompt":
            ...
            domain_prompts = [t.repeat(len(x), 1, 1)
                              for t in network.deep_prompt_tokens]
            with PrependPromptDeep(network.featurizer, domain_prompts):
                feat = network.featurizer(x)
        elif algorithm == "DoPrompt":
            all_z, _ = network.forward_first(x)
            hint = all_z.detach()
            all_bias = network.project(
                hint).reshape(-1, network.num_domains, network.prompt_dim)
...
def domain_class_insight(feat_means, feat_cls_means):
    ...
    stats = {'cross_domain': {}, 'in_domain': {}, 'in_domain_in_cls': {},
             'in_domain_cross_cls': {}, 'cross_domain_in_cls': {}}
    ...
    for d1_in, d1_out in zip(in_name, out_name):
        ...
        for c1 in range(num_classes):
            cls_dist += cosine_dist(feat_cls_means[d1_in]
                                    [c1], feat_cls_means[d1_out][c1])
            cnt += 1
        cls_dist /= cnt
    ...
    for d1 in in_name:
        ...
        for c1, c2 in class_pairs:
            cls_dist += cosine_dist(feat_cls_means[d1]
                                    [c1], feat_cls_means[d1][c2])
            cnt += 1
        cls_dist /= cnt
...
if __name__ == "__main__":
    ...
    for name, loader, weights in evals:
        feat_mean, feat_cls_mean = cal_feat_mean(
            algorithm, loader, weights, device, domain=domain_mapping[int(name[3])], algorithm=args.algorithm)
