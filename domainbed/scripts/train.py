...
if __name__ == "__main__":
    ...
    total_params = sum(p.numel()
                       for p in algorithm.parameters() if p.requires_grad)
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                domain_id = int(name[3])
                acc = misc.accuracy(algorithm, loader, weights, device)
                results[name+'_acc'] = acc
                if args.algorithm == 'MutualInformation':
                    loss = misc.loss(algorithm, loader, weights,
                                     device, domain=domain_id)
    ...
    #     if step == 2999:
    #         save_checkpoint(f'best_model_step{step}.pkl')
    #         break
