...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
                   data_dir, task, holdout_fraction, single_test_envs, specify_test_env, hparams):
    args_list = []
    for trial_seed in range(n_trials-1, n_trials):
        for dataset in dataset_names:
            for algorithm in algorithms:
                if specify_test_env is not None:
                    all_test_envs = [[specify_test_env]]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--specify_test_env', type=int, default=None)
    ...
    args_list = make_args_list(
        ...
        specify_test_env=args.specify_test_env,
