...
from domainbed.lib.misc import (
    random_pairs_of_minibatches, ParamDict, MovingAverage, l2_between_dicts, proj, Nonparametric
)
...
class NetDistLogger:
    ...
    def log_dist(self):
        ...
        print(
            f"[step {self.global_iter}] featurizer dist: {featurizer_dist:.5f}, classifier dist: {classifier_dist:.5f}")
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, cfg=None):
        ...
        if self.hparams["lr"] > 0:
            self.optimizer = optimizer(
                [
                    ...
                    ), 'lr': self.hparams["lr_classifier"], 'weight_decay': 1e-5}
                ],
        ...
        model_grad_params = sum(
            p.numel() for p in self.featurizer.parameters() if p.requires_grad)
        print("Total Parameters: {0}\t Gradient Parameters: {1}".format(
            model_total_params, model_grad_params))
        print("tuned percent:%.3f percentage" %
              (model_grad_params/model_total_params*100))
    ...
    def predict(self, x, domain_id=None):
    ...
    def __init__(self, input_shape, num_classes, num_domains,
        ...
        self.disc_opt = torch.optim.Adam(
            (list(self.discriminator.parameters()) +
             list(self.class_embeddings.parameters()) + list(self.classifier.parameters())),
            lr=self.hparams["lr_classifier"],
            weight_decay=self.hparams['weight_decay'],
...
class MTL(Algorithm):
    ...
    def predict(self, x, env=None, domain_id=None):
...
class AbstractCAD(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains,
        ...
        self.classifier = networks.Classifier(
            ...
            num_classes,
            self.hparams['nonlinear_classifier'])
        params = list(self.featurizer.parameters()) + \
            list(self.classifier.parameters())

        # parameters for domain bottleneck loss
        # whether to use bottleneck conditioned on the label
        self.is_conditional = is_conditional
        self.base_temperature = 0.07
        self.temperature = hparams['temperature']
        # whether apply projection head
    ...
    def bn_loss(self, z, y, dom_labels):
        ...
        mask_drop = ~torch.eye(batch_size).bool().to(
            device)  # drop the "current"/"self" example
        mask_y &= mask_drop
        # contain the same label but from different domains
        mask_y_n_d = mask_y & (~mask_d)
        mask_y_d = mask_y & mask_d  # contain the same label and the same domain
        mask_y, mask_drop, mask_y_n_d, mask_y_d = mask_y.float(
        ), mask_drop.float(), mask_y_n_d.float(), mask_y_d.float()
        ...
        if not self.is_conditional:
            ...
            denominator = torch.logsumexp(
                logits + mask_drop.log(), dim=1, keepdim=True)
            ...
            denominator = torch.logsumexp(
                logits + mask_y.log(), dim=1, keepdim=True)
        ...
        def finite_mean(x):
            ...
            mean = torch.where(torch.isfinite(
                x), x, torch.tensor(0.0).to(x)).sum()
            if num_finite != 0:
...
class CAD(AbstractCAD):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CAD, self).__init__(input_shape, num_classes,
                                  num_domains, hparams, is_conditional=False)
...
class CondCAD(AbstractCAD):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CondCAD, self).__init__(input_shape, num_classes,
                                      num_domains, hparams, is_conditional=True)
...
class Transfer(Algorithm):
    ...
    def loss_gap(self, minibatches, device):
        ...
        max_env_loss, min_env_loss = torch.tensor(
            [-float('inf')], device=device), torch.tensor([float('inf')], device=device)
        for x, y in minibatches:
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for _ in range(self.d_steps_per_g):
            ...
            gap = -self.hparams['t_lambda'] * \
                self.loss_gap(minibatches, device)
            gap.backward()
            self.adv_opt.step()
...
class EQRM(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, dist=None):
        ...
        self.register_buffer('alpha', torch.tensor(
            self.hparams["eqrm_quantile"], dtype=torch.float64))
        if dist is None:
...
class HCVP(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams, cfg=None):
        ...
        super(HCVP, self).__init__(
            input_shape, num_classes, num_domains, hparams, cfg)
        self.featurizer = networks.Vit_HCVP(input_shape, self.hparams)
    ...
    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])
        encoded, domain_prompt, task_prompt = self.featurizer(all_x)
        loss = F.cross_entropy(self.classifier(encoded), all_y)

        # Compute contrastive losses
        coarse_prompt_contrastive_loss = self.coarse_prompt_contrastive_loss_func(
            domain_prompt, temperature=self.hparams['hcvp_temperature'])
        # Apply the weight
        coarse_prompt_contrastive_loss *= self.hparams['hcvp_loss_coarse']
        fine_prompt_contrastive_loss = self.fine_prompt_contrastive_loss_func(
            task_prompt, all_y, temperature=self.hparams['hcvp_temperature'])
        fine_prompt_contrastive_loss *= self.hparams['hcvp_loss_fine']
        class_invariance_contrastive_loss = self.class_invariance_contrastive_loss_func(
            encoded, all_y, temperature=self.hparams['hcvp_temperature'])
        # Apply the weight
        class_invariance_contrastive_loss *= self.hparams['hcvp_loss_invariance']

        total_loss = loss + coarse_prompt_contrastive_loss + \
            fine_prompt_contrastive_loss + class_invariance_contrastive_loss

        self.optimizer.zero_grad()
        total_loss.backward()
        self.optimizer.step()

        return {'loss': loss.item(), 'loss_invariance': class_invariance_contrastive_loss.item(), 'loss_coarse': coarse_prompt_contrastive_loss.item(), 'loss_fine': fine_prompt_contrastive_loss.item()}

    def predict(self, x, domain_id=None):
        encoded, _, _ = self.featurizer(x)
        return self.classifier(encoded)

    def coarse_prompt_contrastive_loss_func(self, coarse_prompt, temperature):
        ...
        # Compute domain_ids based on batch size and number of domains
        num_domains = coarse_prompt.shape[0] // self.batch_size
        domain_ids = torch.arange(num_domains).repeat_interleave(
            self.batch_size).to(coarse_prompt.device)

        # Create the mask (samples from the same domain are positive)
        mask = torch.eq(domain_ids[:, None],
                        domain_ids[None, :]).float().fill_diagonal_(0)

        # Compute the loss
        numerator = (sim_matrix * mask).sum(dim=-1) + 1e-7
        denominator = sim_matrix.sum(dim=-1) - sim_matrix.diag() + 1e-7

        coarse_prompt_contrastive_loss = - \
            torch.log(numerator / denominator).mean()

        return coarse_prompt_contrastive_loss

    def fine_prompt_contrastive_loss_func(self, fine_prompt, labels, temperature):
        # Normalize the fine_prompt with shape of [batch_size, num_tokens, prompt_dim]
        normalized_embeddings = F.normalize(fine_prompt, dim=-1)
        # Compute the similarity matrix
        similarity_matrix = normalized_embeddings @ normalized_embeddings.T / temperature
        sim_matrix = torch.exp(similarity_matrix)

        # Generate the domain ids based on the total number of domains
        num_domains = fine_prompt.shape[0] // self.batch_size
        domain_ids = torch.arange(num_domains).repeat_interleave(
            self.batch_size).to(fine_prompt.device)
        ...
        # Compute the loss
        fine_prompt_contrastive_loss = - \
            torch.log(numerator / denominator).mean()

        return fine_prompt_contrastive_loss

    def class_invariance_contrastive_loss_func(self, embeddings, class_labels, temperature):
        ...
        # Create the mask (samples from the same class are positive)
        mask = torch.eq(
            class_labels[:, None], class_labels[None, :]).float().fill_diagonal_(0)

        # Compute the loss
        # ! note, there may be only one specific class label in the batch, so the corresponding row would be zero for numerator and denominator
        numerator = (sim_matrix * mask).sum(dim=-1) + 1e-7
        denominator = sim_matrix.sum(dim=-1) - sim_matrix.diag() + 1e-7
        ...
        if torch.any(ratio <= 0):
            print("Error: non-positive input to the logarithm function")
            return None

        class_invariance_contrastive_loss = -torch.log(ratio).mean()

        return class_invariance_contrastive_loss
