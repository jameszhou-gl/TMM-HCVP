...
def distance(h1, h2):
    ...
    for param in h1.state_dict():
        ...
        # use Frobenius norms for matrices
        dist += torch.norm(h1_param - h2_param) ** 2
...
def accuracy(network, loader, weights, device, name=None, domain=None):
    ...
    with torch.no_grad():
        for x, y in loader:
            x = x.to(device)
            y = y.to(device)
            p = network.predict(x, domain_id=domain)
            if weights is None:
...
def loss(network, loader, weights, device, name=None, domain=None):
    correct = 0
    total = 0
    weights_offset = 0
    ...
    with torch.no_grad():
        for x, y in loader:
            x = x.to(device)
            y = y.to(device)
            loss = network.print_loss(x, y, domain)
    network.train()
...
class GaussianKernel(Kernel):
    ...
    def cdf(self, test_Xs, train_Xs):
        # kernel centred on each observation
        mus = train_Xs
        sigmas = torch.ones(len(mus), device=test_Xs.device) * \
            self.bw      # bandwidth = stddev
        # repeat to allow broadcasting below
        x_ = test_Xs.repeat(len(mus), 1).T
...
class Distribution1D:
    ...
    def create_dist(self):
        if self.dist_function is not None:
            ...
            raise NotImplementedError(
                "No distribution function was specified during intialization.")
...
class Nonparametric(Distribution1D):
    ...
    def estimate_parameters(self, x):
        ...
        if self.use_kde:
            self.kde = KernelDensityEstimator(
                self.data, bw_select=self.bw_select)
